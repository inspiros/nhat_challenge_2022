...
class Graph:
    ...
    Args:
        ...
        - 'h36m' same with ground truth structure of human 3.6 , with 17 joints per frame
    ...
    def graph_link_between_frames(self, base):
        ...
        :return:
        """
        return [((front - 1) + i * self.num_node_each, (back - 1) + i * self.num_node_each) for i in range(self.seq_len)
    ...
    def basic_layout(self, neighbour_base, sym_base):
        ...
        """
        self.num_node = self.num_node_each * self.seq_len
        time_link = [(i * self.num_node_each + j, (i + 1) * self.num_node_each + j) for i in range(self.seq_len - 1)
                     for j in range(self.num_node_each)]
        self.time_link_forward = [(i * self.num_node_each + j, (i + 1) * self.num_node_each + j) for i in
                                  range(self.seq_len - 1)
                                  for j in range(self.num_node_each)]
    ...
    def get_edge(self, layout):
        ...
        if layout == 'h36m':
            ...
            # neighbor_base = [[16, 15], [15, 14], [14, 8],
            #                  [13, 12], [12, 11], [11, 8],
            #                  [3, 2], [2, 1], [1, 0],
            #                  [6, 5], [5, 4], [4, 0],
            #                  [10, 9], [9, 8], [8, 7], [7, 0]]
...
class SubGraph:
    ...
    def graph_link_between_frames(self, base):
        ...
        :return:
        """
        return [((front) + i * self.num_node_each, (back) + i * self.num_node_each) for i in range(self.seq_len) for
                (front, back) in base]
